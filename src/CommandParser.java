...
public class CommandParser implements AutoCloseable {
    ...
    ArrayList<String> requiredFlags = new ArrayList<String>();
    ArrayList<String> optionalFlags = new ArrayList<String>();

    CommandParser(String[] args) throws IOException {
        if (args.length == 0) {
    ...
    private float getFloat(String flag, String prompt, String defaultValue) {
        ...
        try {
            ...
            return parsingError(0/0, String.format("Value for %s %s is not a number", flag, s));
        }
    }
